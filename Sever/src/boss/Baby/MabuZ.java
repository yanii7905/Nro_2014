package boss.Baby;

import boss.Boss;
import boss.BossID;
import boss.BossesData;
import item.Item;
import map.ItemMap;
import player.Player;
import services.ItemService;
import services.Service;
import services.TaskService;
import player.Service.PlayerService;
import utils.Util;

public class MabuZ extends Boss {

    private long lastTimeDrain;

    public MabuZ() throws Exception {
        super(BossID.MABUZ, BossesData.MABUZ);
    }



  @Override
public synchronized int injured(Player plAtt, long damage, boolean piercing, boolean isMobAttack) {
    if (this.isDie()) return 0;

    // Né đòn
    if (Util.isTrue(10, 1000)) {
        this.chat("Xí hụt");
        return 0;
    }

    long hpBefore = this.nPoint.hp;
    this.nPoint.subHP(damage);
    long realDamage = hpBefore - this.nPoint.hp;

    if (this.isDie()) {
        this.setDie(plAtt);
        this.die(plAtt);
    }

    return (int) realDamage; // ✅ SỐ DAMAGE THẬT
}



    // =====================================================
    // GIẢM HP NGƯỜI CHƠI GẦN (GIỐNG Ở DƠ)
    // =====================================================
    private void drainHpNearbyPlayers() {
        int percent = 10;
        int radius = 200;

        if (!Util.canDoWithTime(lastTimeDrain, Util.nextInt(10000, 20000))) return;

        for (Player pl : this.zone.getNotBosses()) {
            if (pl == null || pl.isDie() || pl.nPoint == null) continue;

            if (Util.getDistance(this, pl) <= radius) {
                int subHp = (int) ((long) pl.nPoint.hpMax * percent / 100);
                if (subHp >= pl.nPoint.hp) subHp = pl.nPoint.hp - 1;

                if (subHp > 0) {
                    this.chat("Hehehe 😈");
                    pl.injured(null, subHp, true, false);
                    PlayerService.gI().sendInfoHpMpMoney(pl);
                }
            }
        }
        lastTimeDrain = System.currentTimeMillis();
    }

    @Override
    public void attack() {
        super.attack();
        drainHpNearbyPlayers();
    }

    // =====================================================
    // RƠI ĐỒ – FIX HIỂN THỊ
    // =====================================================
 @Override
public void reward(Player plKill) {
    TaskService.gI().checkDoneTaskKillBoss(plKill, this);

    int x = this.location.x;
    int y = this.zone.map.yPhysicInTop(x, this.location.y - 24);

    // 50% rơi item 568, 50% không rơi gì
    if (Util.isTrue(50, 100)) {
        ItemMap item568 = new ItemMap(
                this.zone,
                (short) 568,
                1,
                x,
                y,
                plKill.id
        );
        Service.gI().dropItemMap(this.zone, item568);
    }
}


}

